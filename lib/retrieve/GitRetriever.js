var debug = require('debug')('gatherbase:retrieve:git');
...
module.exports = function(spec) {
  debug('new instance of GitRetriever', spec);
  ...
  obj.retrieve = function(url, hJobData, log, progress, callback) {
    ...
    var child = exec('git clone --recursive ' + repoUrl + ' ' + destDir, function(err, stdout, stderr) {
      if (err) {
        err.stdout = stdout;
        err.stderr = stderr;
